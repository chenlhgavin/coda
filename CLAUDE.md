...
- **Latest Dependencies**: Always search the web for the latest dependencies or helm charts or resources and their current usage patterns.
...
- **Type Safety**: Use Rust's type system to make illegal states unrepresentable. Encode invariants in types, not runtime checks. Prefer `NonZeroU32` over `u32` when zero is invalid. Use enums for state machines and the type-state pattern for compile-time state enforcement.
- **Error Handling Strategy**: Library crates (`coda-core`, `coda-pm`) use `thiserror` enums with `#[non_exhaustive]` and domain-specific variants. Application binaries (`coda-cli`, `coda-server`) use `anyhow::Result` with `.context()`. Never use `unwrap()`/`expect()` in production code. Chain source errors via `#[from]` and `#[source]`.
- **API Design**: Use `typed-builder` for structs with more than 5 fields. Prefer explicit types over `impl Trait` in public APIs. Make library structs `#[non_exhaustive]` for future extensibility. Implement `Debug` for all types.
- **Composition Over Inheritance**: Use trait objects (`Arc<dyn GitOps>`) for runtime polymorphism. Compose behavior through trait implementations rather than type hierarchies. Prefer message passing (channels) over shared mutable state.
...
- Run tests with `cargo nextest run --all-features` for parallel execution and better output.
