...
pub trait GitOps: Send + Sync {
    ...
    /// Returns the merge-base (common ancestor) of `base` and HEAD.
    ///
    /// Equivalent to `git merge-base <base> HEAD` run inside `cwd`.
    ///
    /// # Errors
    ///
    /// Returns `CoreError::GitError` if the command fails (e.g., no
    /// common ancestor exists).
    fn merge_base(&self, cwd: &Path, base: &str) -> Result<String, CoreError>;

    /// Soft-resets HEAD to `target`, keeping all changes staged.
    ///
    /// Equivalent to `git reset --soft <target>` run inside `cwd`.
    ///
    /// # Errors
    ///
    /// Returns `CoreError::GitError` if the command fails.
    fn reset_soft(&self, cwd: &Path, target: &str) -> Result<(), CoreError>;

    /// Amends the last commit with currently staged changes, keeping
    /// the original message. Skips pre-commit hooks.
    ///
    /// Equivalent to `git commit --amend --no-edit --no-verify` run
    /// inside `cwd`.
    ///
    /// # Errors
    ///
    /// Returns `CoreError::GitError` if the command fails.
    fn commit_amend_no_edit_internal(&self, cwd: &Path) -> Result<(), CoreError>;

    /// Force-pushes a branch to origin using `--force-with-lease` for
    /// safety.
    ///
    /// Equivalent to `git push --force-with-lease origin <branch>` run
    /// inside `cwd`.
    ///
    /// # Errors
    ///
    /// Returns `CoreError::GitError` if the command fails.
    fn push_force_with_lease(&self, cwd: &Path, branch: &str) -> Result<(), CoreError>;
}
...
impl GitOps for DefaultGitOps {
    ...
    fn merge_base(&self, cwd: &Path, base: &str) -> Result<String, CoreError> {
        let output = run_git(cwd, &["merge-base", base, "HEAD"])?;
        Ok(output.trim().to_string())
    }

    fn reset_soft(&self, cwd: &Path, target: &str) -> Result<(), CoreError> {
        run_git(cwd, &["reset", "--soft", target])?;
        Ok(())
    }

    fn commit_amend_no_edit_internal(&self, cwd: &Path) -> Result<(), CoreError> {
        run_git(cwd, &["commit", "--amend", "--no-edit", "--no-verify"])?;
        Ok(())
    }

    fn push_force_with_lease(&self, cwd: &Path, branch: &str) -> Result<(), CoreError> {
        run_git(cwd, &["push", "--force-with-lease", "origin", branch])?;
        Ok(())
    }
}
