...
impl Runner {
    ...
    pub async fn execute(&mut self) -> Result<Vec<TaskResult>, CoreError> {
        ...
        // Replay completed phases so the UI immediately shows their status
        // and the summary bar reflects their accumulated turns/cost.
    ...
    fn commit_coda_state(&self) -> Result<(), CoreError> {
        ...
        if self.git.has_staged_changes(&self.worktree_path) {
            ...
            self.git.commit_internal(&self.worktree_path, &msg)?;
        ...
        if resp.text.is_empty() && resp.tool_output.is_empty() {
                ...
                && spent >= budget_limit
            {
                ...
                return Err(CoreError::AgentError(format!(
                    "Budget exhausted: spent ${spent:.2} of ${budget_limit:.2} budget. \
