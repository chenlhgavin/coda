...
pub struct Runner {
    ...
    gh: Arc<dyn GhOps>,
    /// Original commit list collected before squash for PR body context.
    pre_squash_commits: Vec<CommitInfo>,
}
...
impl Runner {
    ...
    pub async fn execute(&mut self) -> Result<Vec<TaskResult>, CoreError> {
        ...
        let squashed = self.config.git.squash_before_push;
        if squashed && let Err(e) = self.prepare_squash() {
        ...
        if squashed && !self.pre_squash_commits.is_empty() {
            ...
            if self.git.has_staged_changes(&self.worktree_path) {
                self.git
                    .commit_amend_no_edit_internal(&self.worktree_path)?;
            }
            self.git
                .push_force_with_lease(&self.worktree_path, &branch)?;
        } else {
            self.commit_coda_state()?;
    ...
    async fn create_pr(&mut self) -> Result<TaskResult, CoreError> {
        ...
        let squashed = !self.pre_squash_commits.is_empty();
        let commits = if squashed {
            self.pre_squash_commits.clone()
        } else {
            self.get_commits()?
        };
        ...
        let pr_prompt = self.pm.render(
            ...
            minijinja::context!(
                ...
                commits => commits,
                squashed => squashed,
                original_commits => if squashed { &self.pre_squash_commits } else { &commits },
                state => &self.state,
                checks => checks,
    ...
    /// # Errors
    ///
    /// Returns `CoreError::GitError` if `merge-base` or `reset --soft`
    /// fails.
    fn prepare_squash(&mut self) -> Result<(), CoreError> {
        self.pre_squash_commits = self.get_commits()?;
        if self.pre_squash_commits.is_empty() {
            info!("No commits to squash");
            return Ok(());
        }

        let base = &self.state.git.base_branch;
        let merge_base = self.git.merge_base(&self.worktree_path, base)?;
        info!(
            merge_base = %merge_base,
            commits = self.pre_squash_commits.len(),
            "Squashing commits before PR"
        );
        self.git.reset_soft(&self.worktree_path, &merge_base)?;
        Ok(())
    }
